...
class MaskLDM(LatentDiffusion):
    ...
    def get_input(self, batch, bs=None, *args, **kwargs):
        ...
        encoder_posterior = self.encode_first_stage(masked_image_input)
        ...
        if self.training:
            if random.random()>0.9:
                c = self.get_learned_conditioning([""]*batch_size)
                fusion_out = (torch.zeros((batch_size,8*8,1024)).to(self.device),)
            else:
                fusion_out = None # when training, fusion out should calculate in p_losses
        else:
        ...
        return x, dict(cond_prompt=[c], cond_mask=[mask],cond_mask_img_latents = [masked_image_latents],fusion_out=[fusion_out],image_origin=batch["image_crop_224"],mask_aug16=mask_aug16,clip_x=clip_x,prompt_pool=prompt_pool,global_clip_x=global_clip_x,#uncond=dict(cond_prompt=[self.get_learned_conditioning("").repeat([c.shape[0],1,1])], cond_mask=[mask],cond_mask_img_latents = [masked_image_latents],fusion_out=[fusion_out])
    ...
    def apply_model(self, x_noisy, t, cond,  *args, **kwargs):
        ...
        masked_img_latents = torch.cat(cond['cond_mask_img_latents'],1)
        fusion_out = cond['fusion_out'][0][0] # the last hidden state ,cond['fusion_out'] == [(last_hidden_state[:,1:-1,:], pooled_output,label_pooled)] 
        mask = torch.cat(cond['cond_mask'],1).unsqueeze(1).float()
    ...
    def configure_optimizers(self):
        ...
        # diffusion_params = self.new_add_params
        ...
        opt = torch.optim.AdamW([
            {"params":diffusion_new_params,"lr":fusion_lr},
            {"params":diffusion_params,"lr":diffusion_lr}
            ])
        scheduler = {
